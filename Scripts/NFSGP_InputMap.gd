...
func get_throttle_input() -> float:
	...
	if Input.is_action_pressed("forward"):
		throttle = -1.0 #faking values since keyboard is used instead of joystick
...
func get_brake_input() -> float:
	...
	if Input.is_action_pressed("back"):
		brake = 1.0 #faking values since keyboard is used instead of joystick

	return process_input(brake, brake_mode, brake_inverse)

func get_ebrake_input() -> float:
	var ebrake : float = 0.0
	if Input.is_action_pressed("ebrake"):
		ebrake = 1.0
	return ebrake
...
func get_shift_down() -> bool:
	...
	if Input.is_action_just_pressed("downshift"):
		if !was_gear_down:
			was_gear_down = true
			return true
	else:
...
func get_shift_up() -> bool:
	#VehicleBody3D/GearShift_AudioStreamPlayer2D
